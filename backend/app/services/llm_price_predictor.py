...
- 엔카진단 차량은 검증된 상태 정보가 있으므로 가격 신뢰도가 높습니다.
- "사고이력" 열도 확인하세요: "무사고"는 프리미엄, 사고이력이 있으면 그만큼 감가합니다.
- 소매가 산정 시 엔카진단 차량의 가격을 우선적으로 참고하세요.
...
### 수출/내수 구분 (★ 중요 ★)
- 수출 차량은 내수 대비 시세가 크게 다릅니다 (특히 20만km 이상에서 수출이 33% 높음)
- 대상차량이 내수인 경우: 수출 유사차량 데이터는 참고만 하고 가격 산정에서 제외하세요
- 대상차량이 수출인 경우: 내수 유사차량 대비 상당한 프리미엄이 붙을 수 있습니다
...
```json
{
  ...
  "confidence": "높음",
  "auction_reasoning": "낙찰가 분석 근거를 3~5문장으로 설명 (헤이딜러 데이터 기반)",
  "retail_reasoning": "소매가 분석 근거를 3~5문장으로 설명 (엔카 데이터 기반)",
  "auction_factors": [
...
- auction_factors: 낙찰가에 영향을 미치는 요인 (헤이딜러 데이터 기반). impact는 만원 단위
- retail_factors: 소매가에 영향을 미치는 요인 (엔카 데이터 기반). impact는 만원 단위
- auction_reasoning: 낙찰가 산정 근거 (헤이딜러 유사차량 분석 기반)
- retail_reasoning: 소매가 산정 근거 (엔카 유사차량 분석 기반)
...
def _build_user_message(
    ...
    return (
        ...
        f"대상차량은 {'내수' if domestic else '수출'} 차량입니다. 유사차량 중 {'수출' if domestic else '내수'} 차량 데이터는 참고만 하세요."
    )
