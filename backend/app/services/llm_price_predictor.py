...
### 수출/내수 구분 (★ 매우 중요 ★)
- **수출 낙찰가는 거의 항상 내수 낙찰가보다 높습니다** (해외 수요로 인한 프리미엄). estimated_auction_export ≥ estimated_auction이 정상입니다.
...
- **수출가가 내수보다 낮으면 안됩니다.** 수출 유사차량 데이터가 고주행/구형이라도 대상차량 조건으로 보정하면 내수 이상이 되어야 합니다.
...
def predict_price(
    ...
    # 수출가가 내수보다 낮으면 보정 (수출 프리미엄)
    if export_price > 0 and final_auction > 0 and export_price < final_auction:
        export_price = round(final_auction * 1.05)
        export_reasoning_final += f"\n(수출가가 내수보다 낮아 내수×1.05로 보정)"
