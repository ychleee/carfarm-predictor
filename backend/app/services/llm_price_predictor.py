...
## ★ 가격 산정 원칙 (최우선) ★
- **유사차량 데이터 중 합리적인 범위 내에서 최저가 쪽으로 가격을 산정하세요.**
- 평균이나 중앙값이 아닌, 하위 25% 부근(1사분위~중앙값 사이)을 기준점으로 잡으세요.
- 단, 명백한 이상치(사고차, 수출차 등으로 비정상적으로 낮은 가격)는 제외하고, 정상 거래 범위의 하한선을 기준으로 삼으세요.
